...
SKIPPED_PACKS = ['NonSupported', 'ApiModules', 'Base']
...
def test_playbooks(content_path):
    packs_root, packs, _ = next(os.walk(f'{content_path}/Packs'))
    for pack_dir in packs:
        if pack_dir in SKIPPED_PACKS:
            continue
        if os.path.isdir(f'{packs_root}/{pack_dir}/TestPlaybooks'):
            playbooks_root, _, playbooks = next(os.walk(f'{packs_root}/{pack_dir}/TestPlaybooks'))
...
def test_pack_zip(content_path, target):
    ...
    for playbook_path, playbook in test_playbooks(content_path):
        print(f'\r{playbook_path}', end='')
        with open(playbook_path, 'r') as test_playbook:
            playbook_data = test_playbook.read()
            if not (playbook.startswith('playbook-') or playbook.startswith('script-')):
                playbook_dict = yaml.safe_load(test_playbook)
                playbook_target = f'test_pack/TestPlaybooks/{find_type(_dict=playbook_dict)}-{playbook}'
            else:
                playbook_target = f'test_pack/TestPlaybooks/{playbook}'
        zip_file.writestr(playbook_target, playbook_data)
    zip_file.close()
